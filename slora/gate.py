...
class CountSketchProjector:
    ...
    def __init__(self, d: int, m: int, seed: int, device: torch.device):
        ...
        self.bucket = torch.randint(low=0, high=m, size=(d,), generator=g, dtype=torch.long).to(device)
        self.sign = ((torch.randint(0, 2, (d,), generator=g, dtype=torch.int8) * 2 - 1).to(torch.int8)).to(device)
        self.m = m
        self.device = device
...
class HeadGradientGate:
    ...
    def __init__(
        ...
        self.sign_h = ((torch.randint(0, 2, (d_hidden,), generator=g, dtype=torch.int8) * 2 - 1).to(torch.int8)).to(device)

        g_e = torch.Generator(device='cpu').manual_seed(seed + 1)
        self.bucket_e = torch.randint(0, m, (vocab_size,), generator=g_e, dtype=torch.long).to(device)
        self.sign_e = ((torch.randint(0, 2, (vocab_size,), generator=g_e, dtype=torch.int8) * 2 - 1).to(torch.int8)).to(device)
    ...
    def embed(self, hidden_states: torch.Tensor, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        ...
        batch_idx = torch.arange(N, device=s_e.device).unsqueeze(1).expand(-1, idx.size(1)).reshape(-1)
